...
During the past two week, you learned a bit about how to use `for` loops and `apply` family of functions. Now we are going to do some in class practice and extend your understanding of how they work. In case you want some alternative tutorials [here](https://www.datacamp.com/community/tutorials/tutorial-on-loops-in-r#gs.FgFVHlY) is one that goes into more detail.
...
x
```
2nd fct gives an error because x has not been defined in a global environment but only in the body of the function.
...
system.time(for (x in 1:10000) {print(x ^ 2)})
...
storing it in a vector runs faster because the numbers are already present and the system only has to calculate them.
     User      System verstrichen 
       9.11        0.19        9.30 

#4. What is a very "`R`" to make this (question 2 and 3) even faster? (i.e. think about vectorization)

x <- system.time(for (i in 1:10000) {print(i ^ 2)})
       User      System verstrichen 
       8.96        0.19        9.21 
